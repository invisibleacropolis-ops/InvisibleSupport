...
function renderImage(image) {
    ...
    if (linkInput) linkInput.value = image.blobUrl || image.downloadUrl || '';
    if (openLink) {
        const url = image.blobUrl || image.downloadUrl;
        openLink.href = url || '#';
        if (url) openLink.removeAttribute('aria-disabled'); else openLink.setAttribute('aria-disabled', 'true');
    }
